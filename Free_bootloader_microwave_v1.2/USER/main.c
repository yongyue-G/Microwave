...
static void key1_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        {
            /* 记录按下开始时间 */
            ticksStart = xTaskGetTickCount();//记录时钟节拍数，每1ms记录一次，此时时钟节拍数加1
                ...
                if(KEY1!= 0)
                {
                    ...
                    if(!longPressHandled && pressDuration <= 1000)
                    {
                        ...
                        if(microwave.door_state)/*如果为关门状态*/
                        {
							...
							/*重置状态*/
							/*if微波炉已经完成了一个烹饪循环，此时再次按下按键关门时，需要重置状态机*/
...
void EXTI15_10_IRQHandler(void)
{
	...
	if(EXTI_GetITStatus(EXTI_Line12)!=RESET)
	{
	  ...
	  /* 如果需要的话进行一次任务切换 */
	  portYIELD_FROM_ISR(xHigherPriorityTaskWoken);//触发pendSV中断，然后在该中断完成任务切换
	}
}
/***---------------------------------------------------------------------------------------------------------------------------------------------***/
...
#endif
/***---------------------------------------------------------------------------------------------------------------------------------------------***/
/*1。接收APP；2.拷贝，将ram上的数据copy到flash上；3.跳转到APP*/
