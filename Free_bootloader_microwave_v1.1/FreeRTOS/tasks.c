...
/*FreeRTOS中用于计算时钟节拍计数值的函数，该函数返回统计的时钟节拍计数，if设置的时钟节拍频率位1000Hz，
则一个节拍为1ms--不甘心的咸鱼注*/
...
BaseType_t xSwitchRequired = pdFALSE;
	...
	if( uxSchedulerSuspended == ( UBaseType_t ) pdFALSE )
	{
		...
		if( xConstTickCount == ( TickType_t ) 0U )/*此处代表溢出，因为tickcount每次进systick中断就会加1，
			而xtickcount是u32(无符号32位)类型的，最大值为0xFFFFFFFF,当达到最大值时，此时再次加1会溢出为0--不甘心的咸鱼注*/
		{
